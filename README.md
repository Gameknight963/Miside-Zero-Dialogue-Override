...
You can use this mod to load your own packs. In `OnInitializeMelon`, set `Mod.PacksEnabled` to false. This will prevent it from loading packs from the mszdlg folder. 

> Custom dialogue pack loading runs in `OnLateInitializeMelon`, so it is ok to disable it in `OnInitializeMelon`. Note that any loaded packs will not unload retroactively when you set `PacksEnabled` to false.

To load your own, call `Mod.LoadMszdlg()`:

```csharp
Mod.LoadMszdlg("your path here")
```
